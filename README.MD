...
The server starts on port 8080 by default. Send requests to it like any OpenAI API:
...
| `/health` | GET | Proxy status, model health, budget info |
| `/stats` | GET | Routing distribution, cost breakdown, model usage |

Response headers include `X-Router-Model` and `X-Router-Tier` for debugging which model handled the request and how it was selected.
...
|----------|---------|-------------|
| `ROUTER_PORT` | `8080` | Server listen port |
...
200 tests across 12 test files covering all layers: database, routing engine (tiers 1-3), backend adapters (OpenAI + Anthropic), HTTP endpoints, health monitoring, budget tracking, and end-to-end integration. All tests use in-memory SQLite for isolation.
